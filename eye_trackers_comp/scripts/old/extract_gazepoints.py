...
def main(argv):
    ...
    for s in SURFACE.keys():
        csvfile = ("data/%s/%s/surfaces/gaze_positions_on_surface_%s.csv" % (id,figure,SURFACE[s]))
        data = pd.read_csv (csvfile)
    ...
    with open(csvfile, 'w', newline='') as csvfile:
        ...
        spamwriter.writerow(["Timestamp","Step id", "Action id", "Surface id", "x", "y", "Confidence"])
        for g in gazepoints:
